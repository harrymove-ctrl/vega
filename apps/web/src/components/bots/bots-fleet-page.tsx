...
export function BotsFleetPage() {
  ...
  const [deployGuideStatus, setDeployGuideStatus] = useState<SoDEXOnboardingStatus>({
    ready: false,
  ...
  async function runSingleAction(bot: BotFleetItem, action: Exclude<BotAction, "delete">) {
    ...
    if (action === "deploy" && walletAddress) {
      try {
        await assertSoDEXDeployReadiness(walletAddress, getAuthHeaders);
      } catch (actionError) {
        if (actionError instanceof SoDEXReadinessError) {
          setDeployGuideOpen(true);
        }
        setFeedback({
          tone: "error",
          message: actionError instanceof Error ? actionError.message : "SoDEX readiness check failed",
        });
        return;
      }
    }

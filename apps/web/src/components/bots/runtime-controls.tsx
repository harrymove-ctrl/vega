...
import {
  SoDEXOnboardingChecklist,
  type SoDEXOnboardingStatus,
} from "@/components/sodex/onboarding-checklist";
...
export function RuntimeControls({
  ...
  const { authenticated } = useVegaAuth();
  ...
  const [setupStatus, setSetupStatus] = useState<SoDEXOnboardingStatus>({
    ready: false,
  ...
  const readinessBlocker = useMemo(() => {
    ...
    if (!walletAddress) {
      return "Connect the wallet you want Vega to trade with.";
    }
    if (policyLoading) {
    ...
    if (readinessStatus === "error") {
      return "Unable to verify SoDEX readiness right now.";
    }
  ...
  async function invoke(action: "deploy" | "pause" | "resume" | "stop") {
    ...
    try {
      if (action === "deploy") {
        await fetchSoDEXReadiness(walletAddress, getAuthHeaders).then((payload) => {
          setReadiness(payload);
          if (!payload.ready) {
            throw new SoDEXReadinessError(payload);
          }
        });
      }
      ...
      if (action === "deploy" && runtimeError instanceof SoDEXReadinessError) {
        setSetupOpen(true);
      }

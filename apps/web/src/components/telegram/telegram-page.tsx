...
export function TelegramPage() {
  ...
  const messagePreviews = [
    ...
    },
    {
      ...
      body: "Mean Revert failed place market order on BTC. Reason: SoDEX rejected the order.",
      accent: "#9fd3ff",
    },
    {
      ...
      {status && !status.webhook_ready ? (
        <section className="mb-8 rounded-[1.6rem] border border-[rgba(84,180,255,0.18)] bg-[rgba(48,125,184,0.08)] px-6 py-5">
          ...
          <p className="max-w-3xl text-sm leading-7 text-neutral-300">
            Incoming Telegram messages only reach Vega after the backend has both <code className="rounded bg-white/5 px-1.5 py-0.5 text-[0.75rem]">TELEGRAM_BOT_TOKEN</code> and <code className="rounded bg-white/5 px-1.5 py-0.5 text-[0.75rem]">TELEGRAM_WEBHOOK_URL</code>. Point the webhook URL at <code className="rounded bg-white/5 px-1.5 py-0.5 text-[0.75rem]">/api/telegram/webhook</code> on the FastAPI server, then restart.
          </p>
        </section>
      ) : null}

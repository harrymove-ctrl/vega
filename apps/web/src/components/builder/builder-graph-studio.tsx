...
import { type SoDEXOnboardingStatus } from "@/components/sodex/onboarding-checklist";
...
const BUILDER_CANVAS_STORAGE_KEY = "vega-builder-canvas-v1";
...
function parseMarketScopeSelection(scope: string) {
  ...
  if (lower.includes("all sodex")) return [];
...
function summarizeMarketScope(selectedSymbols: string[], availableSymbols: string[]) {
  if (availableSymbols.length > 0 && selectedSymbols.length === availableSymbols.length) {
    return "All SoDEX spot orderbook";
  }
  if (selectedSymbols.length === 0) {
    return "Choose markets";
  }
  if (selectedSymbols.length <= 3) {
    return `SoDEX spot orderbook / ${selectedSymbols.join(", ")}`;
  }
  return `${selectedSymbols.length} SoDEX markets`;
}
...
function resolveLoadedSelectedMarkets(
  ...
  if (marketScope.trim().toLowerCase().includes("all sodex")) {
...
export function BuilderGraphStudio({
  ...
  onboardingStatus: SoDEXOnboardingStatus;
  ...
  function parsePortableDraft(payload: unknown): PortableBuilderDraft {
    if (!isRecord(payload) || payload.format !== "vega-builder-draft" || payload.version !== 1) {
      throw new Error("This file is not a supported Vega bot draft.");
    }
                ...
                {chatMessages.map((message) => (
                  ...
                  >
                    <div className="mb-1 text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-neutral-500">
                      {message.role === "assistant" ? "Vega AI" : "You"}
                    </div>

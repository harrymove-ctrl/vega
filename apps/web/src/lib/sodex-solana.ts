...
export type SoDEXCluster = "mainnet-beta" | "devnet";

export type SoDEXSolanaConfig = {
  networkLabel: string;
  cluster: SoDEXCluster;
  rpcUrl: string;
  programId: string;
  centralState: string;
  sodexVault: string;
  sodexFallback: string;
...
const MAINNET_CONFIG: SoDEXSolanaConfig = {
  networkLabel: "SoDEX",
  ...
  sodexVault: "72R843XwZxqWhsJceARQQTTbYtWy6Zw9et2YV4FpRHTa",
  sodexFallback: "CRi2Vry6mRjPreRDJxJaJzEpguLoJ9S4o6WR7Et7Ure6",
  collateralMint: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v",
  stableCoinSymbol: "USDC",
  supportsMint: false,
};

const TESTNET_CONFIG: SoDEXSolanaConfig = {
  networkLabel: "Devnet",
  cluster: "devnet",
  ...
  sodexVault: "5SDFdHZGTZbyRYu54CgmRkCGnPHC5pYaN27p7XGLqnBs",
  sodexFallback: "Dz4qRHHfkH94yK1EjwN9hPKEvs3JWXYxK7J8D3TjFgpA",
...
export function getSoDEXSolanaConfig(network: string): SoDEXSolanaConfig {
...
export async function buildMintTestCollateralTransaction(
  ...
  owner: PublicKey,
  config: SoDEXSolanaConfig,
  amount: number,
): Promise<Transaction> {
  if (!config.supportsMint) {
    throw new Error("Minting test collateral is only available on SoDEX devnet.");
  }
...
export async function buildDepositTransaction(
  ...
  const sodexVault = new PublicKey(config.sodexVault);
  ...
  const instruction = new TransactionInstruction({
    ...
    keys: [
      ...
      { pubkey: userCollateralAta, isSigner: false, isWritable: true },
      { pubkey: centralState, isSigner: false, isWritable: true },
      { pubkey: sodexVault, isSigner: false, isWritable: true },
      { pubkey: TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
      { pubkey: ASSOCIATED_TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
